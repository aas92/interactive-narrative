...
(function() {
    ...
    function makeHeart(el, info, pivots) {
        ...
        var cv = el.querySelector('.th-canvas'),
            ctx = cv.getContext('2d');
        var sp = el.querySelector('.th-spark'),
            sctx = sp.getContext('2d');
        ...
        var cur = {
            st: 'pre',
            bpm: info.pre.bpm,
            trace: info.pre.trace,
            times: info.pre.times,
            color: PALETTE.glow,
            wash: PALETTE.wash,
            washLo: PALETTE.washLo,
            lo: 60,
            hi: 90
        };
        var beatPhase = 0,
        ...
        function size() {
            ...
            var hw = Math.min(colW, 470),
                hh = Math.round(hw * 1.05);
            cv.width = hw * DPR;
            cv.height = hh * DPR;
        ...
        function setState(st) {
            ...
            cur.wash = pal.wash;
            cur.washLo = pal.washLo;
            cur.color = pal.glow;
            // stable y-range for the time-series axis (computed once per state)
            var vmn = Math.min.apply(null, cur.trace),
                vmx = Math.max.apply(null, cur.trace);
            if (!isFinite(vmn) || (vmx - vmn) < 4) { vmn = cur.bpm - 6;
                vmx = cur.bpm + 6; }
            var padv = Math.max(2, (vmx - vmn) * 0.18);
            cur.lo = Math.floor(vmn - padv);
            cur.hi = Math.ceil(vmx + padv);
            traceI = 0;
        }
        ...
        function botanicalHeart(c, cx, cy, s, squeeze) {
            ...
            if (squeeze > 0.01) {
                heartShape();
                c.fillStyle = hexA('#ffffff', squeeze * 0.12);
                c.fill();
            }
            ...
            c.fillStyle = hl;
            c.fillRect(X(-9), Y(-10), 18 * sx, 22 * sy);
            c.restore();
        }
        ...
        function drawSpark() {
            ...
            var mL = 48,
                mR = 12,
                mT = 14,
                mB = 42; // margins for the axes/labels
            var plotW = w - mL - mR,
                plotH = h - mT - mB;
            ...
            for (var t = 0; t < ticks.length; t++) {
                ...
                sctx.fillText(ticks[t], mL - 7, yy);
            }

            // y-axis title (rotated)
            sctx.save();
            sctx.translate(12, mT + plotH / 2);
            sctx.rotate(-Math.PI / 2);
            sctx.textAlign = 'center';
            sctx.textBaseline = 'middle';
            sctx.fillStyle = labelCol;
            sctx.fillText('bpm', 0, 0);
            sctx.restore();

            // L-shaped axis lines
            sctx.strokeStyle = axisLine;
            sctx.lineWidth = 1;
            sctx.beginPath();
            sctx.moveTo(mL, mT);
            sctx.lineTo(mL, mT + plotH);
            sctx.lineTo(w - mR, mT + plotH);
            sctx.stroke();
            ...
            for (var xt = 0; xt < xPct.length; xt++) {
                ...
                sctx.lineWidth = 1;
                sctx.beginPath();
                sctx.moveTo(xx, mT + plotH);
                sctx.lineTo(xx, mT + plotH + 5);
                sctx.stroke();
                sctx.fillStyle = labelCol;
                sctx.fillText(xPct[xt] + '%', xx, mT + plotH + 18);
            }
            ...
            if (N >= 2 && tm && tm.length === N) {
                ...
                sctx.strokeStyle = hexA(cur.color, 0.16);
                sctx.lineWidth = 1.2;
                sctx.lineJoin = 'round';
                sctx.beginPath();
                for (var i = 0; i < N; i++) {
                ...
                var k = Math.floor(prog),
                    f = prog - k;
                var leadT, leadV;
                if (k >= N - 1) { leadT = tm[N - 1];
                    leadV = tr[N - 1]; } else { leadT = tm[k] + (tm[k + 1] - tm[k]) * f;
                    leadV = tr[k] + (tr[k + 1] - tr[k]) * f; }

                // the revealed (growing) portion of the line
                sctx.strokeStyle = hexA(cur.color, 0.9);
                sctx.lineWidth = 1.8;
                sctx.beginPath();
                ...
                sctx.beginPath();
                sctx.arc(lx, ly, 6, 0, Math.PI * 2);
                sctx.fill();
                sctx.fillStyle = cur.color;
                sctx.beginPath();
                sctx.arc(lx, ly, 3, 0, Math.PI * 2);
                sctx.fill();
            }
        }
